...
async def cards(date: Optional[str] = Query(None, description="Slate date YYYY-MM-DD")):
    ...
    if not pred_path.exists():
        ...
        if pred_path.exists():
            ...
            if not _has_any_odds_df(tmp):
                try:
                    predict_core(date=date, source="web", odds_source="oddsapi", snapshot=snapshot, odds_best=True)
                except Exception:
                    pass
    ...
    html = template.render(date=date, rows=rows)
...
async def recommendations(
    ...
    html = template.render(date=date, rows=rows, min_ev=min_ev, top=top, markets=markets, bankroll=bankroll, kelly_fraction_part=kelly_fraction_part)
    return HTMLResponse(content=html)
