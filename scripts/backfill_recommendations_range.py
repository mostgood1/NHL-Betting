...
def _recompute_edges_and_recommendations_local(date_str: str, min_ev: float = 0.0) -> None:
    ...
    try:
        ...
        pass
    # Ensure EVs for main and special markets
    for i, r in df.iterrows():
        # Compute and assign EVs individually to avoid issues with duplicate columns
    ...
    # Persist EVs back to predictions
    ...
    # Generate recommendations (never bet against model; tie-break by EV)
    recs: list[dict] = []
    ...
    pd.DataFrame(recs).sort_values("ev", ascending=False).to_csv(outp, index=False)


def _backfill_settlement_for_date_local(date_str: str) -> dict:
    ...
    for i, r in df.iterrows():
        try:
            ...
            # Totals result using available line
            ...
            # First-10 and period totals via PBP
                    ...
                    for p_ in plays:
                        try:
                            ...
                            if per == 1 and (mm is not None and ss is not None):
                                secs = mm * 60 + ss
                                # Heuristic explained in app: treat timeRemaining by default
...
def _reconcile_extended(date_str: str, flat_stake: float = 100.0) -> dict:
    ...
    _write_json(PROC_DIR / f"reconciliation_{date_str}.json", {"summary": summary, "rows": rows})
...
async def process_day(date_str: str, verbose: bool = False) -> dict:
    ...
    # 3) Recompute EVs/edges and persist recommendations (local implementation)
    try:
        _recompute_edges_and_recommendations_local(date_str, min_ev=0.0)
        summary["edges_recs"] = True
    except Exception as e:
        if verbose:
            print(f"[backfill] recompute edges/recs failed {date_str}: {e}")
        summary["edges_recs"] = False
    # 4) Backfill settlement including First-10 and period totals via PBP (local)
    try:
        r = _backfill_settlement_for_date_local(date_str)
    ...
    # 5) Reconciliation including First-10/periods (local)
    try:
        rec = _reconcile_extended(date_str, flat_stake=100.0)
